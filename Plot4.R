...
hpc12$Global_reactive_power <- as.numeric(hpc12$Global_reactive_power)
hpc12$Voltage <- as.numeric(hpc12$Voltage)
hpc12$Global_intensity <- as.numeric(hpc12$Global_intensity)
...
## Prepare layout of graph

par(mfrow = c(2, 2))

## Draw Plot upper left

with(hpc12,plot(DateTime,Global_active_power,type="l",xlab="",ylab = "Global Active Power"))

## Draw Plot upper right

with(hpc12,plot(DateTime,Voltage,type="l",xlab="datetime",ylab = "Voltage"))

## Draw Plot lower left
...
legend(x="topright", col = c("black","red", "blue"), legend = c("Sub_metering_1", "Sub_metering_2","Sub_metering_3"), lty=1,bty="n", cex=0.95)

## Draw Plot lower right

with(hpc12,plot(DateTime,Global_reactive_power,type="l",xlab="datetime"))
